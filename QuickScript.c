...
// ------------------
    strcpy(pattern, "");
    strcpy(userquery, "");
    strcpy(prevuserquery, "");
    strcpy(previoususerquery, "");
    strcpy(secondprevioususerquery, "");
...
void conversation_engine( char *filename )      /* .......... Match and respond to user query .......... */
{
    ...
    MATCHQUERY:
/*
// CHECK FOR EXTERNAL LEARNING AGAIN HERE BECAUSE OF POSSIBLE SRAI (FIRST CHECKED IN RUN QUICKSCRIPT):
// ---------------------------------------------------------------------------------------------------
...
// ----------------------------------
    ...
    if(strcmpi(userquery, "exit")==0)   //Though "exit" is previously checked in run_quickscript(), it may come up here as the result of a SRAI like "== EXIT".
        goto BACKTOHOME;
...
// ---------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    {
        if(strcmpi(filenames[i], EXTERNAL_KNOWLEDGE_TEMP_FILE)==0)  //Temp file of EXTERNAL_KNOWLEDGE_FILE would exist if EXTERNAL_KNOWLEDGE_FILE was included already, otherwise not.
        {
            ispresent=1;
            break;
        }
